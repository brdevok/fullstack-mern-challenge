...
import { EditProfileForm, EditProfileFormValidations, EditProfileSubmitReponse, GetEditableChildProfileData, GetEditableParentProfileData } from "../../../../../types/EditProfile";

const EditChildForm:React.FC = ():JSX.Element => {

    const validate = new Validator({lang: "es"});

    const { id } = useParams() as { id:string };

    const [form, setForm] = useState<EditProfileForm>({ name: "", surname: "", document: "", password: "" });
    ...
    const submit = async () => {
        ...
        const res = await axios.put<EditProfileSubmitReponse>(`/api/children/${id}`, form);
    ...
    const getChildrenData = async () => {
        ...
        const res = await axios.get<GetEditableChildProfileData>(`/api/children/${id}`);
        ...
        <Box sx={{
            ...
            dataLoaded
                ? <Stack sx={{ width: "100%", maxWidth: "350px" }} textAlign="center">
                    <Typography variant="h4" color="primary" mb={2}>Edit child</Typography>
